...
RSpec.describe Werewolf do
  ...
  it 'consumes a victim' do
    werewolf = Werewolf.new('Elizabth', 'UK')
    victim = Victim.new
    werewolf.change!
    expect(werewolf.consume?(victim)).to be true
  ...
  it 'cannot consume a victim if it is in human form' do
    ...
    werewolf.change!
    werewolf.change!
    expect(werewolf.consume?(victim)).to be false
  end
